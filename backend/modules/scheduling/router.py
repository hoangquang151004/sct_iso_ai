...
def list_events(
    ...
    status: str | None = Query(None, pattern="^(SCHEDULED|COMPLETED|CANCELLED|OVERDUE)$"),
    date_from: date | None = None,
    date_to: date | None = None,
    limit: int = Query(100, ge=1, le=1000),
) -> list[CalendarEventResponse]:
    """List calendar events with optional filters."""
    return CalendarEventService.list_events(org_id, assigned_to, event_type, status, date_from, date_to, limit)
...
@router.get("/events/summary/dashboard", response_model=CalendarEventSummary)
...
# =============================================================================
@router.get("/events/{event_id}/reminders", response_model=list[ReminderConfigResponse])
...
@router.post("/events/{event_id}/reminders", response_model=ReminderConfigResponse, status_code=status.HTTP_201_CREATED)
def create_reminder(event_id: UUID, payload: ReminderConfigCreate) -> ReminderConfigResponse:
    """Create a reminder config for an event."""
    return ReminderConfigService.create_reminder(payload)


@router.get("/reminders/{reminder_id}", response_model=ReminderConfigResponse)
...
@router.patch("/reminders/{reminder_id}", response_model=ReminderConfigResponse)
def update_reminder(reminder_id: UUID, payload: ReminderConfigUpdate) -> ReminderConfigResponse:
...
def list_notifications(
    ...
    return NotificationLogService.list_notifications(org_id, user_id, is_read, channel, limit)


@router.post("/notifications", response_model=NotificationLogResponse, status_code=status.HTTP_201_CREATED)
...
@router.get("/notifications/{notification_id}", response_model=NotificationLogResponse)
...
@router.patch("/notifications/{notification_id}/read", response_model=NotificationLogResponse)
...
# =============================================================================
@router.get("/prp-audit-schedules", response_model=list[PRPAuditScheduleResponse])
def list_prp_audit_schedules(
    org_id: UUID | None = None,
    assigned_to: UUID | None = None,
    status: str | None = Query(None, pattern="^(SCHEDULED|IN_PROGRESS|COMPLETED|OVERDUE)$"),
    ...
    return PRPAuditScheduleService.list_schedules(org_id, assigned_to, status, frequency, date_from, date_to)


@router.post("/prp-audit-schedules", response_model=PRPAuditScheduleResponse, status_code=status.HTTP_201_CREATED)
def create_prp_audit_schedule(payload: PRPAuditScheduleCreate) -> PRPAuditScheduleResponse:
    """Create a new PRP audit schedule."""
    return PRPAuditScheduleService.create_schedule(payload)


@router.get("/prp-audit-schedules/{schedule_id}", response_model=PRPAuditScheduleResponse)
...
@router.patch("/prp-audit-schedules/{schedule_id}", response_model=PRPAuditScheduleResponse)
def update_prp_audit_schedule(schedule_id: UUID, payload: PRPAuditScheduleUpdate) -> PRPAuditScheduleResponse:
...
@router.delete("/prp-audit-schedules/{schedule_id}", status_code=status.HTTP_204_NO_CONTENT)
...
@router.post("/prp-audit-schedules/{schedule_id}/complete", response_model=PRPAuditScheduleResponse)
